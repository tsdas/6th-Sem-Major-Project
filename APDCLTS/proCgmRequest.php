<?php
	...
	if(!isset($_POST["mat"]))
	{
		header("location:viewRdetailsCgm.php?selected=null&id=".$reqno);
		return;	
	}
	...
	if(isset($_POST["submit1"]))
	{
		...
		foreach($iqty as $iq)
		{
			...
			if($iq!=0)
			{
				...
				$str6="Select stock from storage where dept='CGM' and mid='$mid'";
				$rs=mysqli_query($link,$str6);
				$r=mysqli_fetch_array($rs);
				$q=$r[0];
				$q=$q-$iq;
				$str7="Update storage set stock='$q' where dept='CGM' and mid='$mid'";
		...
		if($x==1)
		{
			header("location:viewRdetailsCgm.php?qty=null&id=".$reqno);
			return;	
		}
		else
		{
			header("location:viewRdetailsCgm.php?ok=1&id=".$reqno);
		}
