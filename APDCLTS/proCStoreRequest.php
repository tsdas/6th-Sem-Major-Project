<?php
	...
	if(!isset($_POST["mat"]))
	{
		header("location:viewRdetailsCStore.php?selected=null&id=".$reqno);
		return;	
	}
	...
	if(isset($_POST["submit1"]))
	{
		...
		if($x==1)
		{
			header("location:viewRdetailsCStore.php?qty=null&id=".$reqno);
			return;	
		}
		else
		{
			//$str4="update issue set	status='Issued' where issueno='$issueid'";
			//mysqli_query($link,$str4);
			header("location:viewRdetailsCStore.php?ok=1&id=".$reqno);
		}
...
if(isset($_POST["submit2"]))
	{
		...
		{
			$rq=$rqty[$i];
			$str="Select stock from storage where mid='$mid' and dept='Central Store'";
			...
			{
				$str5="Update reqdetails set status='CGM' where rno='$reqno' and mid='$mid'";
				mysqli_query($link,$str5);
				header("location:viewRdetailsCStore.php?is_ok&id=".$reqno);
			}
		}
		if($c==1)
		{
			header("location:viewRdetailsCStore.php?cgm=error&id=".$reqno);
		}
	}
?>