<?php
	...
	if(!isset($_POST["mat"]))
	{
		header("location:viewRdetailsDiv.php?selected=null&id=".$reqno);
		return;	
	}
	...
	if(isset($_POST["submit1"]))
	{
		...
		foreach($iqty as $iq)
		{
			...
			if($iq!=0)
			{
				...
				$str6="Select stock from storage where dept='$divn' and mid='$mid'";
				$rs=mysqli_query($link,$str6);
				$r=mysqli_fetch_array($rs);
				$q=$r[0];
				$q=$q-$iq;
				$str7="Update storage set stock='$q' where dept='$divn' and mid='$mid'";
		...
		if($x==1)
		{
			header("location:viewRdetailsDiv.php?qty=null&id=".$reqno);
			return;	
		}
		header("location:viewRdetailsDiv.php?ok=1&id=".$reqno);
	...
	if(isset($_POST["submit2"]))
	{
		...
		{
			$rq=$rqty[$i];
			$str="Select stock from storage where mid='$mid' and dept='$divn'";
			...
			{
				$str5="Update reqdetails set status='Circle' where rno='$reqno' and mid='$mid'";
				mysqli_query($link,$str5);
				header("location:viewRdetailsDiv.php?is_ok&id=".$reqno);
			}
		}
		if($c==1)
		{
			header("location:viewRdetailsDiv.php?circle=error&id=".$reqno);
		}
	}
?>